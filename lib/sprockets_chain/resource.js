module.exports = (function() {
  ...
  `a` can stub out any of them because it is an ancestor of them all
  `b` can't stub out anything because it has no children
  `c` could stub out test/d because it is a parent
  
  Final take-away: only `a` or `c` could stub out `test/d`
  */
  function collectStubs( deps, parentStubs ){
    ...
    // Collect the paths of our stub directives
    deps.forEach(function(d){
      if ( d.require_directive === "stub" ){
        if ( !stubs ){
          // Slight optimisation - only create a new object if we have 
          // stubbed assets. That way we don't have a long prototype
          // chain of empty objects.
    ...
    // If we don't have any stubs defined ourselves then just return
    // `parentStubs` through directly.
    if ( !stubs ){
      stubs = parentStubs;
    }
    ...
    pathsInTree: function( dir ) {
      ...
      entries.forEach(function( e ) {
        if ( _fs.statSync( _path.join( full_dir, e ) ).isFile() ) {
          ...
          /************************************************************************
           * 2016-03-10 Cori
           * Work around issue with `require_tree` paths resolving incorrectly.
           */
          var finalPath;
          
          // Find the base path containing this asset and trim that path from the front.
          ...
          if (newRoot){
            // Trim newRoot from curFullPath
            finalPath = _path.relative(newRoot, curFullPath);
          } else {
            // Error?
            
            // Fall back to the original path behaviour.
